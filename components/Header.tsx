...
export default function Header() {
  ...
  useEffect(() => {
    const getSession = async () => {
      ...
      if (error) console.error("getSessionError:", error.message)
          ...
          {links.map((link, index) => {
            return (
              <Link
                key={index}
                href={link.href}
                className={`${pathName === link.href ? "bg-pp px-4 py-2 rounded-full text-white" : ""} text-lg text-black font-semibold w-fit`}
              >
                {link.label}
              </Link>
            )
            ...
            {links.map((link, index) => {
              return (
                <Link
                  ...
                  href={link.href}
                  className={`${pathName === link.href ? "bg-pp px-4 py-2 rounded-full text-white" : ""} ml-5 text-lg text-black font-semibold w-fit`}
                >
                  {link.label}
                </Link>
              )
        ...
        <div className='flex items-center gap-4'>
          ...
          {readyCart ?
            <Link href={"/cart"} className='relative cursor-pointer'>
              ...
              <span className='absolute -top-9 text-base text-black bg-pp/10 w-8 h-8 rounded-full flex items-center justify-center'>
          ...
          {loading ? (
            ...
            <Link href={"/auth"} className="bg-pp flex items-center justify-center gap-2 text-white text-base font-medium rounded-full py-3 px-4">
              Login
              <CiUser className='text-2xl' />
            </Link>
          ) : (
            <button onClick={logOut} className="cursor-pointer bg-pp flex items-center justify-center gap-2 text-white text-base font-medium rounded-full py-2 px-4">
              LogOut
